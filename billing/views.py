...
def payment_method_createview(request):
	if request.method == "POST" and request.is_ajax():
		...
		if token is not None:
			#stripe.Customer.retrieve_source("cus_GzcupOKGS4kMmM","card_1GRdekDfQlqHJycnRpyD2JnK",)
			#stripe.Customer.create_source("cus_GzcupOKGS4kMmM",source="tok_amex",)
			# customer = stripe.Customer.retrieve(billing_profile.customer_id)
			# card_response = customer.create_source(billing_profile.customer_id, source=token)
